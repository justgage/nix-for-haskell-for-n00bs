...
Let's talk about Cabal. Cabal is great for a build system but `cabal-install` as a package manager it's got issues. `cabal-install` is known for :fire: **Cabal Hell** :fire:  where you get your packages in a state that's broken and very hard to fix. This is further made worse by the fact that __there's no way to uninstall packages.__ Not to mention that you have to wait for all the dependencies to build just to see if it is even going to work at all. This is __annoying__ and unbecoming of a package manager written in a language with such a central focus on _purity_ and _correctness_. Worst of all and often stops 

I was quite shocked and annoyed when I went to `cabal install` _the most popular haskell frameworks_ such as <http://www.yesodweb.com/> and Snap even libraries like `wreq` **don't work!**. This is mainly because their dependencies' versions are off and the current versions that you have previously `cabal install`'d don't play nice with what a certain library needs. This is because `cabal install` is global. While this can sort of be fixed by `cabal sandbox` this is less than ideal because it wastes space like crazy, takes forever because it builds everything for every new project, and again __doesn't always work.__ 
